...
Last Updated: April 2026 | Version 25
...
- **Cache identity documentation has been corrected to match current code:** the client local audio cache no longer includes DSP in the key, and the server OGG cache no longer includes `model_version`. This document now includes a dedicated section describing the current cache identity rules and known weak points so future hardening work has an explicit baseline.
...
- Renders a separate RRVB font barcode identity side-channel for unit GUID and NPC name: `§RRVX-G=<guid>;N=<name>§`. This side-channel is secret-safe in Lua: the addon only concatenates literal prefix/suffix strings around secret values and does not inspect, measure, or transform secret payload content.
...
- Also scans the RRVB identity side-channel. The monitor locates the framed RRVB panel by thresholding, flood-filling edge-connected light regions, finding 4-connected interior panel components, and decoding candidate panels with the OpenCV RRVB detector.
...
| Stage | Description |
|---|---|
| RvBarcodeMonitor | Runs continuous region polling plus periodic full-screen relocation scans, decodes valid RV QR packets, and fires `OnPacketDecoded` per non-preview packet. It also locates and decodes the RRVB identity side-channel (`RRVX-G=<guid>;N=<name>`) so GUID/name metadata can be recovered even when QR name fields are unavailable in combat. |
| TtsSessionAssembler | Collects chunks by DIALOG ID. Fires `OnSegmentComplete` when all chunks arrive. Applies NPC race override before resolving voice slot. Carries player-name metadata (`PlayerName`, `PlayerRealm`, `PlayerClass`) through assembled segments so the playback path can perform cache-preserving replacement and remote batch planning later. Fires `OnSessionReset` on DIALOG ID change. Sets `AppServices.LastSegment` after each completion. |
| PlaybackCoordinator | Dequeues segments, checks cache (`TryGetDecodedAsync` returns `PcmAudio`), resolves remote batch submissions when a segment was expanded into client-planned subsegments, and feeds `PcmAudio` to `PlaylistPlayAsync`. In `WaitForFullText` mode it waits on the final post-split audible segment count, not just the assembler's original count. |
...
### 5.3 Player-Name Replacement and Remote Batch Expansion

Player-name handling is now part of the normal synthesis path rather than an experimental option. The goal is to keep most of a line cacheable while isolating the player-dependent text into its own segment.

Current behavior:
- All replacement modes use the same sentence-based cache-preserving split flow: actual player name, cache-friendly title, class replacement, with or without realm suffix.
- Tiny bridge fragments around the name are intentionally avoided. Earlier approaches such as splitting down to a few surrounding words proved unstable for Chatterbox-family models.
- The split target expands to the full containing sentence, and if that sentence is too short it may expand to two sentences.
- Default settings are: `Player name handling = Use cache-friendly title`, `Replacement title = Champion`, `Append realm as "of <Realm>" = false`.

When the active provider is remote and the split produces multiple pieces, the client submits them as one explicit remote batch chain. Each batch piece receives a synthetic segment ID (`seg_0`, `seg_1`, …) and each later piece points at the immediately prior piece as its continuity source. This low-level chain is intentionally one same-voice stream, even when the split occurred for cache-preserving reasons rather than natural sentence boundaries.
...
### 7.5 Voice Identity Namespace

The resolved voice identity used by synthesis and cache lookup incorporates the slot string as a namespace prefix:
...
The current client local audio cache key is a SHA-256 hash of fields joined by null bytes (`\x00`):
...
Important current-code detail: the method signature still accepts a `dspKey`, but DSP is intentionally excluded from the hash. DSP is a client-side post-process applied after cache retrieval, so changing DSP settings must not invalidate the local synthesized-audio cache.

`voiceId` here is the resolved voice identity string, not just a raw sample name. For remote/sample-based paths it is a slot-namespaced identity such as `Narrator:<identity>` or `Amani/Female:<identity>` so two slots using the same sample do not collide locally.

### 10.3 Current Cache Identity Rules (Client and Server)

This section documents the current code behavior, not the desired future hardened behavior. It is intentionally explicit because cache-key hardening is the next planned work area.

#### Client local audio cache (L1)

Current key inputs:
- raw segment text (or reconstructed phrase text for phrase-level storage)
- resolved voice identity string
- provider ID

Current resolved voice identity fields (`BuildIdentityKey()`):
- `VoiceId`
- `LangCode`
- `SpeechRate`
- `CfgWeight`
...
Current formatting/normalization behavior on the client:
- most numeric identity fields are formatted to 2 decimals
- `VoiceInstruct` and `CosyInstruct` are trimmed
- `LongcatGuidance` is trimmed and lowercased
- `VoiceId` casing is preserved as-is, so casing differences can still fork local cache identity
...
```
normalized_text \x00 provider_id \x00 voice_identity \x00 lang_code \x00 speech_rate \x00 cfg_weight \x00 exaggeration \x00 cfg_strength \x00 nfe_step \x00 cross_fade_duration \x00 sway_sampling_coef \x00 voice_context
```

Important current-code detail: `model_version` is no longer part of the server OGG cache key. Provider subdirectories isolate the cache by backend, and replacing model artifacts currently requires clearing provider cache entries manually.

Current server `voice_identity` rules:
- base voice → raw `voice_id`
- reference voice → content hash of the reference sample
- description voice → hash of the description text
- blend voice → canonical sorted blend identity

Current server `voice_context` behavior:
- starts with the slot identity string so two slots using the same text/sample do not collide
- also absorbs several cache-affecting controls today, including `voice_instruct`, `cosy_instruct`, `lux_num_steps`, `lux_t_shift`, `lux_return_smooth`, `longcat_steps`, `longcat_cfg_strength`, `longcat_guidance`, `cb_temperature`, `cb_top_p`, `cb_repetition_penalty`, and resolved seed
- is also used as part of prior-token gating for Chatterbox-family continuation behavior

Current server formatting/normalization behavior:
- `speech_rate` uses 2 decimal places
- `cfg_weight`, `exaggeration`, `cfg_strength`, `cross_fade_duration`, and `sway_sampling_coef` use 3 decimal places
- many values folded into `voice_context` use 2 decimal places
- the server key uses normalized text, not raw submitted text

#### Additional server-side cache identities

These are distinct from the main OGG cache and should remain conceptually separate:
- **conditioning cache** for reference/blend conditioning artifacts
- **tail-token sidecar** (`.tokens.pt`) keyed alongside the OGG cache key for Chatterbox-family T3 continuation

Known current weak points:
- client and server do not normalize text in the same place or the same way
- client and server do not use identical numeric precision rules
- several server controls are still packed into the freeform `voice_context` string
- stale assumptions such as “DSP affects client audio cache” or “model_version is in server cache keys” are now incorrect

This section should be revised again when cache-key hardening work begins.
...
### 12.2 Current Settings Defaults Relevant to Playback and Caching
...
- In the main Settings UI, **Audio Output Device** appears above **Speed**
...
### 12.5 Pronunciation Tab (Right Column)
...
### 12.6 Speak/Action Button Wrapping
...
### 12.7 Voice Profiles — Import / Export
...
### 12.9 Voice Profile Editor Workflow

The current voice-profile editor is organized around the common workflow of **set voice → preview → tweak → preview again**. The top of the dialog keeps the common controls fixed and places the less frequently used sections below in a scrollable region.

Current intended layout:
- **fixed top area:** voice controls, language, speech rate, live preview state
- **scrollable lower area:** applies/accent info, warning banner, standard setup/restore standard, advanced render controls, DSP, summary

Current wording/layout details carried in the implementation and recent fixes:
- seed help text should describe blank as provider-default behavior, not random-by-definition
- `Summary` and `Restore Standard` live near the bottom
- `Audio Effects (DSP)` uses the full width of the lower section
- the live preview status text sits above the Preview/Stop buttons
...
### 15.6 RRVB Identity Side-Channel

The QR protocol remains authoritative for dialog text. RRVB is a separate on-screen identity side-channel used to carry unit GUID and NPC name metadata. The current application payload shape is:

```text
§RRVX-G=<unit-guid>;N=<npc-name>§
```

The RRVB symbology itself is generic Latin-1 byte encoding. `RRVX`, `G=`, and `N=` are application-level fields parsed by `RvBarcodeMonitor`; `RrvBarcodeDetector` must not filter or depend on those prefixes.

Current RRVB font baseline:

| Property | Current value |
|---|---|
| Font | `RuneReaderBarcode-Regular.ttf` |
| Font version | 3.4 / v10 advance-250, 50/200/50 guards |
| WoW font size | 13pt target |
| Glyph advance | 250 em |
| Drawn glyph width | 300 em |
| Left guard | 50 em |
| Data zone | 200 em, 8 horizontal lanes |
| Right guard | 50 em, intentionally overlaps next glyph's left guard |
| Start/stop | `§` sentinel with left/center/right full-height vertical bars |
| Binary threshold | 10, `ThresholdTypes.BinaryInv` |

The 300-em drawn width with 250-em advance is intentional. WoW FontString rendering showed visible gaps with fully contained 250-em glyphs; the v10 overlap makes adjacent guards merge into one reliable 50-em boundary span.

Current monitor acquisition model:

1. Threshold frame to an RRVB ink mask using value 10 and `BinaryInv`.
2. Invert the ink mask to produce a light-panel mask.
3. Flood-fill all light regions connected to the image edge to black.
4. Run `ConnectedComponentsWithStats` with `PixelConnectivity.Connectivity4` on remaining interior light panels.
5. Validate candidate panels by checking for barcode-like ink inside them.
6. Crop exact panel bounds; no padding is added.

Current detector model:

1. Threshold candidate crop using RRVB threshold 10 and `BinaryInv`.
2. Find vertical guard spans, group them into rows by Y-overlap, and sort rows/glyphs geometrically.
3. Detect the `§` sentinel triplet as start/stop.
4. Decode row-major cells between sentinels. For each cell, sample the center of the data gap to avoid guard bleed.
5. Divide row height into 8 relative lane slots. Each lane uses a peak-row search inside the slot to tolerate 1-pixel WoW raster jitter.
6. Decode bytes as Latin-1 and return raw text to the monitor.
7. Stop scanning the frame after the first closed RRVB block.

Debug image dumps and verbose RRVB traces are disabled by default. When enabled for troubleshooting, detector images are written under `<AppContext.BaseDirectory>\rrvb-debug`.
...
- **Cache identity hardening:** current client/server cache behavior is documented, but it is still somewhat fragile. Text normalization boundaries differ, numeric precision rules differ, and the server still packs several cache-affecting controls into `voice_context`. Hardening this is a planned next step.
...
### 21.3c Airgapped / Offline Deployment
...
All providers accept the same core request shape. The server validates that the requested `voice.type` is supported by the named provider and returns a clean error if not.
...
Optional synthesis controls currently present in the server/client contract:
- Chatterbox-family: `cfg_weight`, `exaggeration`, `cb_temperature`, `cb_top_p`, `cb_repetition_penalty`, `synthesis_seed`
...
- Qwen/other instruction-capable backends: `voice_instruct`
- LongCat: `longcat_steps`, `longcat_cfg_strength`, `longcat_guidance`

`voice.type` values: `"base"` (voice_id), `"reference"` (sample_id), `"blend"` (blend array), and `"description"` (voice_description, Qwen-Design only).

`speech_rate` is a hint in the range `[0.5, 2.0]`. Providers may clamp internally. CosyVoice explicitly clamps to `[0.5, 2.0]` and forwards the value as `speed`.

#### Batch request model

The v2 batch endpoint accepts a list of batch segments. Each segment carries the same synthesis fields as a normal request plus batch-specific metadata:
- `segment_id`
- `voice_context`
- `continue_from_segment_id` (server-side field name) / `prime_from_segment` (client-side JSON field name)

The client uses this to submit same-stream continuity chains for cache-preserving name/title splits and other explicit remote batch flows.

### 21.8a Chatterbox-Family Large-Text Handling

`chatterbox`, `chatterbox_full`, and `chatterbox_multilingual` now handle large text transparently on the server. Oversized requests are split server-side at sentence boundaries, fall back to clause boundaries when a sentence is still too large, and only resort to hard word-boundary splitting as a last resort. The backend synthesizes each piece in sequence and concatenates the PCM before OGG encoding, so the client still receives one OGG result for one request.

This is separate from client-requested v2 batch splitting. The internal large-text splitter exists so long narration can still succeed even when the client sends one large request. The client-requested batch path exists for cache-preserving and continuity-aware segmentation decisions such as player-name replacement.

### 21.9 Server Cache Key and Identity

The current server OGG render cache key is a SHA-256-based identity truncated for storage efficiency. It is computed from:
...
- `model_version` is **not** part of the server OGG cache key anymore
- text is normalized before key generation
- several additional controls are currently folded into `voice_context` rather than appended as separate structured fields

`voice_identity` is:
- For `base`: the `voice_id` string
- For `reference`: a content hash derived from the reference sample
- For `description`: a hash of the description text
- For `blend`: canonical sorted blend identity

This is the current implementation baseline and should be treated as authoritative over older document text.
...
### 21.10c Tail-Token Sidecars and Continuation

Chatterbox-family backends maintain a separate `.tokens.pt` sidecar alongside cached OGG files for T3 continuation. This sidecar is not the same thing as the conditioning cache and must not be conflated with it.

Current behavior:
- sidecars are keyed alongside the main OGG cache identity
- single-chunk requests can persist tail tokens to disk
- explicit continuation requests first prefer in-memory prior tokens and then fall back to loading the persisted sidecar from disk
- the sidecar payload includes the saved token tail plus `voice_key` and `voice_context` guards to avoid cross-voice/cross-slot contamination

### 21.10d Client-Requested Batch Continuity and Batch Join Trim

The v2 batch route supports explicit client continuity links between related segments. `continue_from_segment_id` lets the server resolve one batch segment against the cache identity of a prior segment in the same batch.

This exists for a different reason than the server's internal long-text handling:
- internal Chatterbox-family large-text splitting is sentence-based and should sound natural on its own
- client-requested batch splits can occur for cache-preserving reasons, including mid-sentence name/title isolation
- those client splits therefore need explicit continuity carried from one returned batch item to the next

After all batch items are synthesized, the server rejoins the returned audio. For `chatterbox`, `chatterbox_full`, and `chatterbox_multilingual`, the batch handoff layer can trim a small amount from the tail of each non-final batch item using `RRV_CB_BATCH_JOIN_TAIL_TRIM_MS` (default 100 ms). This trim happens only at the client-requested batch rejoin layer; it intentionally does not alter the backend's internal sentence-level chunking logic.
...
```
POST /api/v1/synthesize/v2                                 — submit single job, returns {progress_key, cache_key, cached}
POST /api/v1/synthesize/v2/batch                           — submit explicit multi-segment batch
...
RuneReaderVoice v25 reflects the current client/server baseline after the post-v24 work on player-name replacement, wait-for-full-text, remote batch continuity, Chatterbox-family large-text handling, batch join tail trim, cache-identity documentation, and RRVB v10 identity side-channel hardening. The document now describes not only the provider/backend roster and benchmark tooling, but also the current runtime rules that the client and server follow when they split text, chain batch segments, generate cache identities, and rejoin returned audio.
