...
rrv-server/
  ...
  .env.example
  SETUP.md
  INSTALL_UBUNTU.md       # Ubuntu 24.04 + CUDA install guide with confirmed steps
  server/
    ...
    backends/
      ...
      kokoro_backend.py
      f5tts_backend.py    # includes Vocos local path, trailing padding fix
      chatterbox_backend.py  # includes CPU float32 monkey-patches
    routes/
      health.py
      capabilities.py
      providers.py
  ...
  models/                 # model files (gitignored, volume-mounted in container)
    MODELS.txt            # download locations and file lists for all models
  samples/                # reference audio clips (gitignored, volume-mounted)
    SAMPLES.txt           # naming conventions, sidecar format, auto-conversion docs
    originals/            # original files moved here after ffmpeg conversion
  cache/                  # generated OGG files (gitignored, volume-mounted)
```

Installing only what you need:
```bash
# Core — Kokoro only (CPU, fast)
pip install -e ".[kokoro]"

# Add F5-TTS and Chatterbox Turbo (CUDA)
...
pip uninstall torchcodec -y   # transformers 5.x ships torchcodec which conflicts

# System dependency
sudo apt install ffmpeg -y
```

See `INSTALL_UBUNTU.md` for the complete Ubuntu 24.04 + CUDA setup guide with confirmed versions.

### 21.3c Sample Intake Pipeline

Reference audio samples are the input for voice-matching synthesis (F5-TTS, Chatterbox Turbo). The server manages the full lifecycle automatically:
...
Any WAV file without a `.ref.txt` transcript sidecar is transcribed automatically using a locally pre-placed Whisper model (never downloaded at runtime). The transcript is written as `<stem>.ref.txt` alongside the audio. F5-TTS and Chatterbox both require this transcript — synthesis is refused if it's absent. Whisper is loaded lazily and unloaded immediately after transcription to free VRAM. Language is auto-detected by Whisper — no forced language constraint.

**3. Auto-profiling (librosa signal analysis)**
After transcription, the audio is analysed using librosa signal processing and a one-line voice description is written as `<stem>.txt`. The description format is:
```
Gender · Register · Tone · Texture · Energy · Language · Pace · Duration
```
Example: `Male · Bass · Dark tone · Clear · Moderate energy · English · Slow pace · 8.3s`

The profiler runs independently of Whisper — existing samples with `.ref.txt` but no `.txt` are profiled on every polling pass without reloading Whisper. This allows threshold recalibration by deleting `.txt` sidecars and restarting.

**Signal analysis metrics:**
- **Pitch (F0)** via `librosa.pyin` → gender + vocal register (Bass/Baritone/Tenor/Alto/Mezzo-soprano/Soprano)
- **Spectral centroid** → tone color (Dark/Warm/Neutral/Bright/Piercing)
- **Spectral flatness** → texture (Smooth/Clear/Slightly airy/Airy/Breathy) — thresholds calibrated for TTS synthetic audio
- **RMS energy** → dynamic level (Soft/Moderate/Strong/Powerful)
- **Words-per-minute** from transcript → pace (Very slow/Slow/Moderate/Fast/Rapid)
...
**Known limitation:** Gender detection is ambiguous in the 140–175Hz F0 range where male tenor and female contralto/alto overlap acoustically. This is a real acoustic phenomenon not a classifier error.
