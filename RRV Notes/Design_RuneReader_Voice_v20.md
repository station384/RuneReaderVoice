...
Last Updated: March 2026 | Version 20
...
`sample_id` is the filename stem. Optional `.txt` and `.ref.txt` sidecars provide description and transcript metadata. When a sample family has generated provider clips, the provider-facing sample list should resolve the provider clip and its matching provider-sidecar transcript rather than falling back to the master family transcript.
