...
Last Updated: March 2026 | Version 21

---

## What's New in v21

- **Finalized provider-specific sample output controls:** The server now treats the imported master reference as a fixed **44.1 kHz stereo WAV**, while emitted provider clips can be configured independently by provider for **sample rate** and **channel count**. The intended baseline is **Chatterbox-family = stereo 44.1 kHz** and **F5-TTS = mono 22.05 kHz**, but both are environment-configurable.

- **Generated clip retranscription is now part of the normal intake pipeline:** Whisper runs on generated provider clips and variants, not only on the master sample. Each emitted provider clip receives its own `.ref.txt` transcript sidecar derived from the actual generated audio instead of inheriting sliced text from the master transcript.

- **Provider-specific sample and transcript resolution hardened:** Provider-facing sample enumeration and synthesis now consistently resolve the provider clip and its matching provider transcript sidecar rather than accidentally surfacing the master transcript for a provider clip.

- **Sample extraction correctness and validation improved:** Generated clips are sliced from the preserved master PCM instead of an unintended mono analysis buffer. Extraction validates emitted clip format, enforces provider-specific rate/channel expectations, and rejects invalid outputs when the configured format does not match the written file.

- **Sample processing diagnostics expanded:** Additional debug logging now covers Whisper input/output, sidecar writes, provider sample resolution, emitted clip format, and generated clip retranscription so sample-pipeline failures can be diagnosed from logs alone.

- **Sample throughput tuning added for large backfills:** The Phase 5 sample watcher now supports bounded parallel CPU profiling, deferred generated-clip retranscription, and optional batched Whisper passes for bulk sample imports. These controls are implementation-tunable and may be constrained by available GPU memory.

- **Remote chunk playback is more defensive about format drift:** The client-side remote multi-chunk path now normalizes decoded remote PCM chunks to a common format before concatenation when sample rate or channel count differ across returned chunks.

- **Server OGG encoding now preserves intended output format:** The final ffmpeg OGG encode path explicitly preserves the synthesized PCM sample rate and channel count after loudness normalization so server-rendered OGGs do not drift to unexpected formats such as 192 kHz.

- **Design note for future transcript selection:** Transcript candidates must continue to be preserved exactly as produced. Any future “best transcript” selector must choose an untouched winning candidate rather than rewriting text, because capitalization, whitespace, punctuation, and pause structure materially affect TTS cadence.

- **Provider-family sample naming/reporting contract clarified and enforced:** Provider-generated files on disk are internal artifacts named with a provider family token, e.g. `<base>-f5[-variant].*` and `<base>-chatterbox[-variant].*`, plus `<base>-master.*` for the normalized master. Client-facing `sample_id` values returned by `/api/v1/providers/{id}/samples` are now explicitly provider-neutral logical IDs such as `<base>` and `<base>-slow`. `chatterbox` and `chatterbox_full` are treated as the same sample family for file resolution and enumeration, both resolving against the `-chatterbox` file set.
...
`chatterbox` and `chatterbox_full` are aliases for the same sample family and therefore both resolve against the `-chatterbox` files on disk. The server does not create parallel `-chatterbox_full` sample artifacts.

**5. Generated clip retranscription and profiling**  
Generated provider clips receive their own Whisper transcript sidecars (`.ref.txt`) based on the actual emitted clip audio rather than inheriting sliced text from the master transcript. Generated clip voice-profile sidecars (`.txt`) are then produced from the emitted provider clips.

**6. Validation and diagnostics**  
Extraction validates the emitted clip format and rejects outputs that do not match the configured provider-specific sample rate/channel expectations. The watcher also logs Whisper input/output, sidecar writes, provider clip resolution, emitted clip format, and retranscription activity to make sample-pipeline failures diagnosable from logs.
...
| `RRV_F5_SAMPLE_CHANNELS` | `1` | Channel count for emitted F5 reference clips (`1` or `2`) |
| `RRV_F5_SAMPLE_RATE` | `22050` | Sample rate for emitted F5 reference clips |
| `RRV_CHATTERBOX_SAMPLE_CHANNELS` | `2` | Channel count for emitted Chatterbox-family reference clips (`1` or `2`) |
...
| `RRV_WHISPER_MASTER_BATCH_SIZE` | `1` | Optional batched Whisper size for master/raw sample transcription |
| `RRV_WHISPER_GENERATED_BATCH_SIZE` | `1` | Optional batched Whisper size for generated clip retranscription |
...
- on disk: `<base>-f5[-<variant>].<ext>` and `<base>-chatterbox[-<variant>].<ext>`
- returned to clients: `<base>` and `<base>-<variant>`

Examples:
- `F_NightElf-f5.wav` → `sample_id = "F_NightElf"`
- `F_NightElf-f5-slow.wav` → `sample_id = "F_NightElf-slow"`
- `F_NightElf-chatterbox.wav` → `sample_id = "F_NightElf"`
- `F_NightElf-chatterbox-slow.wav` → `sample_id = "F_NightElf-slow"`

Internal provider-tagged identifiers such as `F_NightElf-f5-slow` or `F_NightElf-chatterbox-slow` must not be surfaced to clients. Optional `.txt` and `.ref.txt` sidecars provide description and transcript metadata. When a sample family has generated provider clips, the provider-facing sample list resolves the provider clip and its matching provider-sidecar transcript rather than falling back to the master family transcript.

Provider-family alias rule:
- `f5tts` / F5 requests enumerate only the `-f5` family
...
RuneReaderVoice v21 extends the v16 PCM-first architecture with a parallel synthesis pipeline, comprehensive NPC voice override capabilities, community sync infrastructure, and a significantly expanded NPC race catalog, while also hardening the Phase 5 sample-intake pipeline for real-world voice-library building.
...
The Phase 5 sample library pipeline now normalizes dropped source media into a 44.1 kHz stereo master WAV, retranscribes generated provider clips with Whisper, writes provider-specific transcript/profile sidecars, and supports provider-specific emitted sample formats (sample rate + channel count) while preserving a single high-quality master. This allows Chatterbox and F5-TTS to consume different reference-clip formats without duplicating the ingest workflow.
The sample-library contract is now explicit: provider-tagged filenames are internal storage artifacts, while provider-facing sample lists expose only provider-neutral logical IDs. `chatterbox` and `chatterbox_full` both resolve against the same `-chatterbox` family on disk.

Bulk sample processing also gained bounded parallel clip profiling, optional batched Whisper passes, and richer pipeline diagnostics so large backfills can be processed faster and debugged from logs without guesswork.
