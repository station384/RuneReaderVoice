...
Last Updated: April 2026 | Version 22
...
RuneReaderVoice v22 extends the PCM-first architecture with a reusable voice-sample defaults layer, more reliable NPC fallback behavior, visible text-shaping defaults, dynamic QR scan pacing, and release-ready licensing/documentation cleanup.
...
The NPC voice override system now supports per-NPC bespoke voice samples with a reusable **sample default** layer. A user can tune a bespoke sample once in the Voice Defaults tab, then reuse that baseline across many NPC IDs. Resolution order is bespoke sample defaults first, then race/dialect slot defaults, then Narrator fallback.

The community sync system continues to poll for NPC override crowd-sourced updates and now also carries all-provider payloads for slot voice profiles and voice sample defaults. All sync operations use Local-wins merge logic where appropriate so local user data is not silently overwritten by server records.

The assembler now also treats whitespace-only segments as completed dialog parts so an empty separator segment cannot stall an entire multi-segment dialog.

The DSP chain remains an ordered `List<DspEffectItem>`. The same editor is now reused for slot profiles and sample-default profiles, with slot-only controls hidden when editing a bespoke sample baseline.
