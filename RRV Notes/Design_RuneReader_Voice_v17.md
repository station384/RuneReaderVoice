...
Status: Phases 1–4 Complete, Phase 5 Server Implemented / Client Integration Complete | Target: Retail WoW

Last Updated: March 2026 | Version 17

---

## What's New in v17

- **Parallel synthesis pipeline:** `PlaybackCoordinator` now fires synthesis tasks for all segments in a dialog concurrently as they arrive from the assembler. Playback begins as soon as segment 0 is ready and continues in strict sequence order. If a later segment finishes synthesis before an earlier one has finished playing, it waits in the task map at zero cost. Buffer underrun is handled naturally by `await`ing the next task. This eliminates the serial synthesize→play→synthesize→play pattern and significantly reduces perceived latency on multi-segment dialogs.

- **Assembler sub-chunk contamination fix:** the early-chunk stash key now includes `SeqIndex` (`MakeEarlyKey(subTotal, flags, race, seqIndex)`). Previously, two segments with the same shape (identical SubTotal, flags, and race) shared a stash, allowing stale sub-chunks from a prior transmission to contaminate a later segment's accumulator. This was the root cause of scrambled book/journal text playback. Non-zero sub matching now also requires `Subs[0] != null` (anchor populated) before accepting a sub into an accumulator.

- **Per-NPC bespoke voice sample override:** the NPC Race Override system now supports a `BespokeSampleId` field (plus optional `BespokeExaggeration` and `BespokeCfgWeight`). When set, the coordinator substitutes the specified reference sample for that NPC's voice synthesis while preserving all other slot settings (DSP, speech rate, accent). The override never applies to narrator segments — narrator slots always use their own voice profile regardless of the NpcId. The Last NPC panel shows a "Voice sample" dropdown (filtered to the active provider's supported sample length) when a voice-matching provider is active. The voice list is cached in memory and warmed at startup via a background fetch.

- **Cache key includes slot identity:** the synthesis cache key now incorporates the slot string (`Narrator:`, `BloodElf/Male:`, etc.) as a prefix. This prevents two different slots that default to the same reference sample from sharing cache entries and playing the wrong voice.

- **Community sync — NPC overrides and server defaults:** the server now exposes crowd-source endpoints for NPC voice overrides (`GET /api/v1/npc-overrides/since`, `POST /api/v1/npc-overrides`, `PUT /api/v1/npc-overrides/{id}`) and seed-data endpoints for the three non-NPC data types (`GET/PUT /api/v1/defaults/{type}`). The client polls for new NPC override records every 5 minutes. On first load (when `FirstLoadComplete = false`) the client pulls all four default types from the server. Manual Push/Pull buttons are available on all four rule tabs. `ContributeByDefault` setting enables automatic background contribution of NPC override saves.

- **NPC override data model extended:** `NpcRaceOverride` now carries `BespokeSampleId`, `BespokeExaggeration`, `BespokeCfgWeight`, `Source`, `Confidence`, and `UpdatedAt` (Unix timestamp for delta sync). `NpcRaceOverrideDb.MergeFromServerAsync` applies Local-wins merge logic — Local entries are never overwritten by server records.

- **23 new NPC race accent groups:** `Amani`, `Arathi`, `Broken`, `Centaur`, `DarkTroll`, `Dredger`, `Dryad`, `Faerie`, `Fungarian`, `Grummle`, `Hobgoblin`, `Kyrian`, `Nerubian`, `Refti`, `Revantusk`, `Rutaani`, `Shadowpine`, `Titan`, `Tortollan`, `Tuskarr`, `Venthyr`, `ZulAman`. All have male/female slot pairs in `NpcVoiceSlotCatalog` with accent labels and Kokoro default sample assignments. These are NPC-only races — they do not appear in the player race ID maps.

- **Chatterbox text preprocessing:** `ChatterboxPreprocess()` runs on all text sent to Chatterbox backends. Converts inline `<annotation text>` to `"... "` (natural pause), cleans dash-reconstructed sentence joins from paragraph splits, and strips orphaned leading/trailing dashes. Eliminates the model hallucination/repetition pattern that occurred when angle-bracket flavor text appeared mid-paragraph.

- **Chatterbox chunk limits tightened:** `TargetChars` reduced from 550 to 380 for Chatterbox Full, from 700 to 600 for Chatterbox Turbo. `RepeatedSentenceLimit` reduced from 3 to 2 for Chatterbox Full. High-exaggeration threshold lowered from `>1.0` to `>=1.0` with more aggressive reduction (70%/75% instead of 75%/80%).

- **HttpClient connection pool hardened:** `RemoteTtsClient` uses `SocketsHttpHandler` with `PooledConnectionLifetime=90s` and `PooledConnectionIdleTimeout=60s`. This prevents Caddy's idle keep-alive timeout from killing connections underneath the pool and eliminates the `IOException(SocketException 995)` noise during idle periods. A global `TaskScheduler.UnobservedTaskException` handler suppresses any remaining pool-internal background exceptions.

- **Reverse proxy support:** `RRV_TRUSTED_PROXY_IPS` config (default `127.0.0.1`) activates `ProxyHeadersMiddleware` so real client IPs appear in server logs when running behind Caddy.

- **Comprehensive diagnostic logging:** `System.Diagnostics.Debug.WriteLine` coverage added throughout the pipeline — assembler packet receipt/accumulation/firing, coordinator synthesis start/cache hit/play start/done, remote provider chunking decisions and bespoke application, NPC sync poll results and merge decisions. All calls are compiled away in Release builds (`[Conditional("DEBUG")]`).
...
Session reset (new dialog ID or ESC) cancels the session CTS, stops the player, and clears `_synthTasks`.
...
- **Row 2:** Voice sample dropdown — visible only when the active provider is a `RemoteTtsProvider` with a populated voice cache. Populated from `GetAvailableVoices()` filtered by provider duration limits (F5-TTS ≤11s, Chatterbox ≤40s). First item is "(use race default sample)". Voice list is cached in memory and warmed at startup via background `RefreshVoiceSourcesAsync`. Save stores `BespokeSampleId` against the NpcId. Clear resets to race default (null).
...
RuneReaderVoice v17 extends the v16 PCM-first architecture with a parallel synthesis pipeline, comprehensive NPC voice override capabilities, community sync infrastructure, and a significantly expanded NPC race catalog.
...
The assembler sub-chunk contamination bug (scrambled book/journal text) is fixed by including `SeqIndex` in the early-chunk stash key and requiring `Subs[0] != null` before accepting non-zero subs. The synthesis cache key now includes the slot string as a prefix, preventing cross-slot cache collisions when two slots default to the same reference sample.

Chatterbox text preprocessing (`ChatterboxPreprocess`) eliminates the model hallucination pattern caused by WoW's inline angle-bracket flavor annotations. Chunk limits are tightened to stay within Chatterbox's reliable operating range. The NPC race catalog is expanded with 23 new non-playable race accent groups, each with male/female slot pairs and Kokoro default sample assignments, replaceable with actual WoW VO extractions via the bespoke sample system.
