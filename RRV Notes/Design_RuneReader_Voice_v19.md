...
Last Updated: March 2026 | Version 19

---

## What's New in v19

- **Server sample intake pipeline overhaul:** Phase 5 sample intake now normalizes dropped source media into a **44.1 kHz stereo master WAV**, pads lead/tail silence during import, archives the original source in `originals/`, and treats that master WAV as the authoritative source for all downstream extraction and analysis. This replaces the older 16 kHz mono intake assumption.

- **Generated clip retranscription:** Whisper now runs on **generated provider clips and variants**, not only the master sample. Each emitted provider clip receives its own `.ref.txt` transcript sidecar derived from the actual generated audio instead of inheriting sliced text from the master transcript.

- **Provider-specific sample format controls:** The sample extractor now supports provider-specific emitted clip formats while preserving a fixed stereo master. Generated provider clips can be configured independently for **sample rate** and **channel count** through environment settings. Current design defaults are **Chatterbox = stereo 44.1 kHz** and **F5-TTS = mono 22.05 kHz**, while the master ingest format remains stereo 44.1 kHz.

- **Provider-specific sample/transcript resolution hardening:** Provider-facing sample enumeration and synthesis now consistently resolve the provider clip and its matching provider transcript sidecar rather than accidentally surfacing the master transcript for a provider clip.

- **Sample extraction correctness fixes:** Generated clips are now sliced from the preserved master PCM instead of an unintended mono analysis buffer. Extraction validates the emitted format and rejects invalid clip outputs when the configured sample rate/channel expectations are not met.

- **Sample processing diagnostics:** Additional debug logging was added around Whisper input/output, sidecar writes, provider sample resolution, and emitted clip formats to make sample-pipeline failures diagnosable from logs alone.

- **Sample throughput improvements:** The Phase 5 sample watcher now supports bounded parallel CPU profiling, deferred generated-clip retranscription, and optional batched Whisper passes for bulk sample backfills. These optimizations are explicitly implementation-tunable and may be constrained by available GPU memory.

- **Design note for future transcript selection:** The design now explicitly preserves transcript candidates exactly as produced. Any future “best transcript” selector must choose an untouched winning candidate rather than rewriting text, because capitalization, whitespace, punctuation, and pause structure materially affect TTS cadence.
