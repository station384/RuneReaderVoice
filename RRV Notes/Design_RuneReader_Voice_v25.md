...
RuneReaderVoice v25 reflects the current client/server baseline after the post-v24 work on player-name replacement, wait-for-full-text, remote batch continuity, Chatterbox-family large-text handling, batch join tail trim, and cache-identity documentation. The document now describes not only the provider/backend roster and benchmark tooling, but also the current runtime rules that the client and server follow when they split text, chain batch segments, generate cache identities, and rejoin returned audio.
