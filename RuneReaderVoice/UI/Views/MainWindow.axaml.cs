...
public partial class MainWindow : Window
{
    ...
    {
            if (ProviderSelector.SelectedItem is ComboBoxItem item && item.Tag is string providerId)
    ...
    private void PopulateVoiceGrid()
    {
        ...
        foreach (AccentGroup group in Enum.GetValues<AccentGroup>())
        {
            ...
            {
                // Narrator has a single slot
                var combo = BuildVoiceCombo(voices, new VoiceSlot(group, Gender.Unknown));
            ...
            {
                var maleCombo   = BuildVoiceCombo(voices, new VoiceSlot(group, Gender.Male));
                var femaleCombo = BuildVoiceCombo(voices, new VoiceSlot(group, Gender.Female));
                Grid.SetColumn(maleCombo,   1);
                Grid.SetColumn(femaleCombo, 2);
                row.Children.Add(maleCombo);
                row.Children.Add(femaleCombo);
            }
...
#endif
        ProviderSelector.Items.Add(new ComboBoxItem { Content = "Local AI Voice (ONNX) — coming soon", Tag = "onnx" });
